...
- **pnpm** 8 or higher (preferred) — install with `npm i -g pnpm`
- **Git**
...
verino/
├── packages/
│   ├── verino/        ← main package (core + vanilla adapter)
│   ├── react/         ← @verino/react
│   ├── vue/           ← @verino/vue
...
```bash
pnpm test                          # run all tests (369 total)
...
- **No DOM imports in `src/core/`** — the core must remain pure and testable in Node.js
- **No cross-adapter imports** — each adapter in `src/adapters/` imports only from `'../core/index.js'`
